...
$categories_query = ambildata($condb, 'SELECT DISTINCT category FROM m2');
...
foreach ($categories as $category) {
    foreach ($days_of_week as $day) {
        $query = "SELECT idmenu FROM m2 WHERE category = '$category' AND hari = '$day'";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    foreach ($_POST['menu'] as $day => $categories) {
        foreach ($categories as $category => $menu_id) {
            // Update m1 table
            $query = "UPDATE m2 SET idmenu = '$menu_id' WHERE hari = '$day' AND category = '$category'";
