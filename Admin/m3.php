...
$categories_query = ambildata($condb, 'SELECT DISTINCT category FROM m3');
...
foreach ($categories as $category) {
    foreach ($days_of_week as $day) {
        $query = "SELECT idmenu FROM m3 WHERE category = '$category' AND hari = '$day'";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    foreach ($_POST['menu'] as $day => $categories) {
        foreach ($categories as $category => $menu_id) {
            // Update m1 table
            $query = "UPDATE m3 SET idmenu = '$menu_id' WHERE hari = '$day' AND category = '$category'";
...
<body>
    ...
    <h1>Jadual Makan Minggu 3</h1>
    <button class="btn btn-primary"><a href="tblmenu3.php" style="text-decoration: none; color:white">Back To Table</a></button>
    <br><br>
